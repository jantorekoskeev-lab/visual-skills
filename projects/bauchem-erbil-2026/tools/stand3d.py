...
class Cam:
    def __init__(self, eye, target, f, size, up=(0, 0, 1)):
        ...
        self.f, self.cx, self.cy = f, size[0] / 2, size[1] / 2
...
def box(d, cam, x0, y0, x1, y1, z0, z1, top="#DFE4EE", lit="#EDEFF5", dark="#C9D1E0"):
    ...
    faces = [
        ...
        ([v(x0, y1, z1), v(x0, y0, z1), v(x0, y0, z0), v(x0, y1, z0)], lit),   # левая,  -X
    ]
...
def cylinder(d, cam, cx, cy, r, z0, z1, n=44):
    ...
    up, lo = ring(z1), ring(z0)
    for i in range(n):
        j = (i + 1) % n
        d.polygon([up[i], up[j], lo[j], lo[i]], fill="#D5DBE8")
    d.polygon(up, fill="#E9EDF4", outline="#A8B2C6")


def chair(d, cam, cx, cy):
    for dx, dy in ((-180, -180), (180, -180), (-180, 180), (180, 180)):
        box(d, cam, cx + dx - 22, cy + dy - 22, cx + dx + 22, cy + dy + 22, 0, 430)
    box(d, cam, cx - 225, cy - 225, cx + 225, cy + 225, 430, 480)      # сиденье
    box(d, cam, cx - 225, cy + 165, cx + 225, cy + 225, 480, 870)      # спинка


def render(path, eye, target, f, size=(2200, 1500)):
    base = Image.new("RGB", size, "#FFFFFF")
    cam = Cam(eye, target, f, size)
